...
function AgentUI({
  ...
  return (
    ...
    >
      {error && (
        <Notice status="error" isDismissible={true} onRemove={() => {
			onResetChat();
			onAgentStart();
		}}>
          {error}
        ...
        <FlexBlock className="big-sky__agent-ui-content">
          ...
          {agentThought && (
            <AgentMessage
              ...
              isActive={!agentQuestion && !agentConfirm && !agentMessage}
            />
          )}
          {agentMessage && (
          ...
          {agentQuestion && (
            <AgentQuestion question={agentQuestion.function.arguments.question}>
              <AskUserQuestion
                {...agentQuestion.function.arguments}
                question=""
                onCancel={() => {
                  setToolCallResult(agentQuestion.id, "(canceled)");
                  onResetTools();
                  onResetChat();
                }}
                onAnswer={(answer, files) => {
                  ...
                  setToolCallResult(agentQuestion.id, answer);
                  userSay(answer, files);
                }}
              />
            </AgentQuestion>
          ...
          {agentConfirm && (
            <AgentQuestion question={agentConfirm.function.arguments.message}>
              <Confirm toolCall={agentConfirm} onConfirm={onConfirm} />
            </AgentQuestion>
          )}
          {!agentMessage && !agentQuestion && !agentConfirm && (
            <AgentThinking running={running} toolRunning={toolRunning} />
          )}
          <ToolNotices />
        </FlexBlock>
      </Flex>
